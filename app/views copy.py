...
def generate_mashups(request):
    ...
    try:
                ...
                for comp in ['full', 'vocals', 'instrumental']:
                    if comp in song['components']:
                        files_to_zip.append({
                            ...
                            'zip_path': f"{solo_folder}/{song['name']}^Song A^{comp.capitalize()}.mp3"
                        })
                output_data.append({
                    'song1': song['name'],
                    'song1_bpm': song['bpm'],
                        ...
                        for comp in ['full', 'vocals', 'instrumental']:
                            ...
                            if comp in s1['components']:
                                files_to_zip.append({
                                    ...
                                    'zip_path': f"{mashup_folder}/{s1['name']}^Song A^{comp.capitalize()}.mp3"
                                })
                            ...
                            if comp in s2['components']:
                                if abs(s2['bpm'] - s1['original_bpm']) <= 22:
                                    ...
                                    adjusted = beatmatch_audio(
                                        s2['components'][comp],
                                        original_bpm=s2['bpm'],
                                        target_bpm=(s1['original_bpm'] + s2['bpm']) / 2
                                    )
                                ...
                                files_to_zip.append({
                                    ...
                                    'zip_path': f"{mashup_folder}/{s2['name']}^Song B^{comp.capitalize()}.mp3"
                                })
        ...
        if output_format == 'csv':
            ...
            writer = csv.DictWriter(response, fieldnames=output_data[0].keys())
            writer.writeheader()
